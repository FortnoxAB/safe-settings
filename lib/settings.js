...
class Settings {
  ...
  async handleResults () {
    ...
    */
    ...
    const changedRepos = new Set(Object.values(stats.changes).flatMap(reposForPlugin => Object.keys(reposForPlugin)))
...
<!-- safe-settings-check-run-id: ${payload.check_run.id} -->

**Repos considered:** ${Object.keys(stats.reposProcessed).length} · **Repos affected:** ${changedRepos.size} · **Errors:** ${Object.keys(stats.errors).length} · **Plugins affected:** ${Object.keys(stats.changes).join(', ') || 'None'}

View the full per-repo breakdown: ${payload.check_run.html_url}
...
`
      ...
      const truncatedBody = summary.length > 55536 ? `${summary.substring(0, 55536)}... (too many changes to report)` : summary
      const body = `${truncatedBody}\n\n- [ ] I have reviewed the changes and verified that they are intended.`

      const comments = await this.github.paginate(this.github.rest.issues.listComments, {
        owner,
        repo,
        issue_number: pullRequest.number,
        per_page: 100
      })
      const matches = comments.filter(comment => comment.body?.includes('#### :robot: Safe-Settings config changes detected:'))
      const previousComment = matches[matches.length - 1]
    ...
    const conclusion = error ? 'failure' : (changedRepos.size === 0 ? 'success' : 'action_required')
